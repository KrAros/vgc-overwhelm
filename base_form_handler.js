		...
		.ready(
				function() {
					...
					$('#loading').fadeIn(
							400,
							function() {
								preloadImages([ 'auroraveil_0_off.png',
										'auroraveil_0_on.png',
										'auroraveil_1_off.png',
										'auroraveil_1_on.png', 'Boost_off.png',
										'Boost_on.png', 'bRMT_off.png',
										'bRMT_on.png', 'brmt-tl.png',
										'doubles_off.png', 'doubles_on.png',
										'dual_off.png', 'dual_on.png',
										'electric_off.png', 'electric_on.png',
										'grassy_off.png', 'grassy_on.png',
										'hail_off.png', 'hail_on.png',
										'helpinghand_0_off.png',
										'helpinghand_0_on.png',
										'helpinghand_1_off.png',
										'helpinghand_1_on.png', 'Import.png',
										'LC_off.png', 'LC_on.png',
										'lightscreen_0_off.png',
										'lightscreen_0_on.png',
										'lightscreen_1_off.png',
										'lightscreen_1_on.png',
										'misty_off.png', 'misty_on.png',
										'mode_hyb_off.png', 'mode_hyb_on.png',
										'mode_ver_off.png', 'mode_ver_on.png',
										'mode_vis_off.png', 'mode_vis_on.png',
										'NU_off.png', 'NU_on.png',
										'OS-1_off.png', 'OS-1_on.png',
										'OS-1_top.png', 'OS-2_off.png',
										'OS-2_on.png', 'OS-2_top.png',
										'OS-3_off.png', 'OS-3_on.png',
										'OS-3_top.png', 'OS-5_on.png',
										'OS-5_top.png', 'OU_off.png',
										'OU_on.png', 'Priority_off.png',
										'Priority_on.png', 'psychic_off.png',
										'psychic_on.png', 'rain_off.png',
										'rain_on.png', 'reflect_0_off.png',
										'reflect_0_on.png',
										'reflect_1_off.png',
										'reflect_1_on.png', 'Reset.png',
										'RU_off.png', 'RU_on.png',
										'sand_off.png', 'sand_on.png',
										'setlevel_5_off.png',
										'setlevel_5_on.png',
										'setlevel_50_off.png',
										'setlevel_50_on.png',
										'setlevel_100_off.png',
										'setlevel_100_on.png', 'swc-tl.png',
										'sun_on.png', 'sun_off.png',
										'trickroom_off.png',
										'trickroom_on.png', 'Uber_off.png',
										'Uber_on.png', 'UU_off.png',
										'UU_on.png', 'VGC_off.png',
										'VGC_on.png', 'WMT_off.png',
										'WMT_on.png', 'wmt-tl.png', ]);
							});
					$('#loading').fadeOut();
					...
					$(".input")
							.on(
									'change keyup keydown paste mouseup autocompleteselect autocompletechange',
									function(event, ui) {
					...
					// on level change
					$('input[name="setLevels"]')
							.on(
									'change',
									function() {
										var newDefaultLevel = $(
												'input[name="setLevels"]:checked')
												.val();
										...
										for (var i = 0; i < environment.pokemons.length; i++) {
											for (var j = 0; j < environment.pokemons[i].length; j++) {
												...
												pokemonFormData
														.find('.pokemon-level')
														.val(
																environment.defaultLevel);
												// trigger change
												pokemonFormData.find(
														'.pokemon-level')
														.trigger('change');
											}
											;
					...
					$('#team1').on(
							'shown.bs.collapse',
							function() {
								if ($("#team1").hasClass('in')
										|| $("#team1").hasClass('collapsing')) {
									$("#collapseTeam1").html('Collapse');
									$("#collapseTeam1").removeClass('btn-info')
					...
					$('#team1')
							.on(
									'hidden.bs.collapse',
									function() {
										if ($("#team1").hasClass('collapsing')
												|| $("#team1").hasClass(
														'collapse')) {
											$("#collapseTeam1").html('Expand');
											$("#collapseTeam1").removeClass(
...
function initialiseAutocomplete() {
	...
	$(".ability").autocomplete(
			{
				source : function(request, response) {
					var results = $.ui.autocomplete.filter(ABILITIES_LIST,
							request.term);
					response(results.slice(0, 15));
				},
				autoFocus : true,
	...
	$(".pokemon-name,.pokemon-item,.ability,.move-0,.move-1,.move-2,.move-3")
			.each(function() {
...
function updateStaticData() {
	for (var i = 0; i < environment.pokemons.length; i++) {
		for (var j = 0; j < environment.pokemons[i].length; j++) {
			...
			var pokemonFormData = $(id);
			// update the HP stat
			var HPvalue = pokemonFormData.find('.stat-0');
			HPvalue.text(getStat(pokemon, STAT_HP));
			// update Atk stat
			var ATKvalue = pokemonFormData.find('.stat-1');
			ATKvalue.text(getStat(pokemon, STAT_ATT));
			// update Def stat
			var DEFvalue = pokemonFormData.find('.stat-2');
			DEFvalue.text(getStat(pokemon, STAT_DEF));
			// update SpA stat
			var SPAvalue = pokemonFormData.find('.stat-3');
			SPAvalue.text(getStat(pokemon, STAT_SPA));
			// update SpD stat
			var SPDvalue = pokemonFormData.find('.stat-4');
			SPDvalue.text(getStat(pokemon, STAT_SPD));
			// update Spe stat
			var SPEvalue = pokemonFormData.find('.stat-5');
			SPEvalue.text(getStat(pokemon, STAT_SPE));
			...
			var genderlessOption = genderDropDown.find('option[value="'
					+ GENDER_GENDERLESS + '"]');
			if (!isValidPokemonName(pokemon.name)
				...
				if (!genderlessOption.length) {
					genderDropDown
							.append('<option value="2">Genderless</option>');
				}
				;
 ...
 * 
 * @param teamNum -
 *            the team number (index of pokemons object)
 */
function updatePokemonTeamForm(teamNum) {
	for (var i = 0; i < environment.pokemons[teamNum].length; i++) {
		...
		for (var stat = 0; stat < 6; stat++) {
			...
			pokemonFormData.find('.statBoost-' + stat).val(
					pokemon.statBoost[stat]);
		}
	}
};
 ...
 * 
 * @param pokemonData
 *            form data for a particular pokemon
...
function validatePokemonData(pokemonData) {
	...
	for (var i = 0; i < DEFAULT_NUM_MOVES; i++) {
		...
		if (isDifferent(moveInput.val(), pokemon.moves[i])
				&& !isValidMoveName(moveInput.val()) && moveInput.val()) {
	...
	if (isDifferent(itemInput.val(), pokemon.item)
			&& !isValidItem(itemInput.val()) && itemInput.val()) {
	...
	if (isDifferent(abilityInput.val(), pokemon.ability)
			&& !isValidAbility(abilityInput.val()) && abilityInput.val()) {
...
/**
 * Checks if two values are different or not. It is a case insensitive check and
 ...
 * @param isError
 *            true if the object should be labelled an error, or false
 *            otherwise.
 */
...
/**
 * Changes the terrain of the battle.
 */
function updateEnvironment(rmt, enviroInput) {
	if (enviroInput && enviroInput.hasClass('terrain')) {
		// we have a terrain change.
		var number = parseInt(enviroInput.attr('id').split('-')[1]);
		if (enviroInput.prop('checked')) {
			// we are changing the environment to non-zero
			environment.terrain = number;
		} else {
			// we change the environment to normal
			environment.terrain = NO_TERRAIN;
		}
		;
		for (var i = 0; i <= 4; i++) {
			var id = '#terrain-' + i;
			if (environment.terrain === i) {
				// check this box
				$(id).prop('checked', true).parent('label').addClass('active');
			} else {
				$(id).prop('checked', false).parent('label').removeClass(
						'active');
			}
			;
		}
	...
	environment.auroraVeilA = $('#auroraveil-0').prop('checked');
	environment.auroraVeilB = $('#auroraveil-1').prop('checked');
	environment.critA = $('#crit-0').prop('checked');
	environment.critB = $('#crit-1').prop('checked');
	environment.helpingHandA = $('#helpinghand-0').prop('checked');
	environment.helpingHandB = $('#helpinghand-1').prop('checked');
	environment.zMoveA = $('#zmove-0').prop('checked');
	environment.zMoveB = $('#zmove-1').prop('checked');
	environment.protectA = $('#protect-0').prop('checked');
	environment.protectB = $('#protect-1').prop('checked');
 ...
 * 
 * @param pokemonData
 *            the Pokemon input data
 * @param isValid
 *            true if the data is valid, or false otherwise.
 */
function updatePokemon(pokemonData, isValid) {
	...
	for (var i = 0; i < 6; i++) {
		...
		if (isDifferent(pokemonData.find('.statBoost-' + i).val(),
				pokemon.statBoost[i]))
			pokemon.changeStatBoost(i, pokemonData.find('.statBoost-' + i)
					.val());
	}
	;
};
...
function getPokemonObject(pokemonData) {
	...
	/*
	 * id will be pokemon-x-y where x = 0 if yours, 1 if opponent 0 <= y <= 5,
	 * the pokemon number.
	 */
	var res = id.split('-');
...
function updateForm(rmt) {
	...
	$('#auroraveil-0').prop('checked', environment.auroraVeilA);
	$('#auroraveil-1').prop('checked', environment.auroraVeilB);
	$('#crit-0').prop('checked', environment.critA);
	$('#crit-1').prop('checked', environment.critB);
	$('#helpinghand-0').prop('checked', environment.helpingHandA);
	$('#helpinghand-1').prop('checked', environment.helpingHandB);
	$('#zmove-0').prop('checked', environment.zMoveA);
	$('#zmove-1').prop('checked', environment.zMoveB);
	$('#protect-0').prop('checked', environment.protectA);
	$('#protect-1').prop('checked', environment.protectB);
	...
	if (rmt) {
		...
		$('input[name="tier"][value="' + tierOptions.tier + '"]').prop(
				'checked', true);
		...
		// this for loop makes everything less than or equal to true but not
		// highest
...
/**
 * Updates a particular cell in the display table. TODO COMPLETE MEEEE!
 */
function updateCell(cell_id, isValid, results, subCell, paintParent) {
	...
	if (!isValid || results[subCell].attacks.length == 0) {
		...
		;
		var cellColor = getCellColor(results[subCell].attacks[0], otherAttack,
				subCell, results[subCell].attackerStrikeFirst);
		...
		case 'visual':
			...
			break;
		case 'verbose':
			text = '<span style="font-size:14px;"><b>' + move
					+ '</b></span><br/>' + '<span style="font-size:12px;">'
					+ percentage_min + '% - ' + percentage_max + '%</span>';
			break;
		default:
			text = '<b>' + move + '</b><br/>'
					+ '<span style="font-size:14px;">' + percentage_min
					+ '% - ' + percentage_max + '%</span>';
			break;
		}
		;
		...
		for (var i = 0; i < results[subCell].attacks.length; i++) {
			...
			var mvtype = MOVE_DATA[results[subCell].attacks[i].move].type;
			if (mv === 'Hidden Power') {
...
/**
 * Updates the Pokemon sets dropdown menu. This is called upon change in
 * Pokemon.
 * 
 * @param pokemon
 *            the Pokemon object.
 * @param pokemonData
 *            the DOM object corresponding to that Pokemon.
 */
function updatePokemonSets(pokemon, pokemonData) {
	...
	for (var i = 0; i <= 7; i++) {
		if (typeof COMMON_SETS[i][pokemonName] !== 'undefined') {
			setsInput.append('<option value="common:' + i + ':' + pokemonName
	...
	/*
	 * if (typeof POKEMON_SETS[pokemonName] !== 'undefined') { for (var i = 0; i <
	 * POKEMON_SETS[pokemonName].length; i++) { setsInput.append('<option
	 * value="' + pokemonName + ':' + i + '">' +

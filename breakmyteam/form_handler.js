...
$(document)
		.ready(
				function() {
							...
							.on(
									...
									function() {
										...
										if ($('#topleftimage').html() !== '<img src="../css/images/brmt-tl.png">') {
											$('#topleftimage')
													.html(
															'<img src="../css/images/brmt-tl.png">');
										}
										;
										$('#topleftimage').css(
					...
					$(".input-enviro").on('click change keyup keydown mouseup',
							function() {
							...
							.on(
									...
									function() {
										...
										if (tierOptions.tier <= 5) {
											...
											// "Check" everything underneath the
											// tier
										...
										if (tierOptions.tier === TIER_TO_NUMBER['vgc']) {
											...
											// set levels to 50
											$('#chkl50').addClass("active");
											$('#chkl5').removeClass("active");
											$('#chkl100').removeClass("active");
										} else if (tierOptions.tier === TIER_TO_NUMBER['lc']) {
											// set levels to 5
											$('#chkl5').addClass("active");
											$('#chkl50').removeClass("active");
											$('#chkl100').removeClass("active");
											$('#doubles')
													.prop('checked', false);
										} else {
											// set levels to 100
											$('#chkl100').addClass("active");
											$('#chkl50').removeClass("active");
											$('#chkl5').removeClass("active");
											// turn off doubles
											$('#doubles')
										...
										;
										tierOptions.boostedSweepers = $(
												'#boostedSweeper').prop(
												'checked');
										tierOptions.enablePriority = $(
												'#enablePriority').prop(
												'checked');
										tierOptions.liteMode = $('#topThreats')
												.prop('checked');
										if ($(this).hasClass('outspeed')) {
											// we have an outspeed change.
											var number = parseInt($(this).attr(
													'id').split('-')[1]);
											if (number != tierOptions.outSpeed) {
												// we are changing the speed to
												// non-zero
											...
											;
											// this for loop makes everything
											// less than or equal to true but
											// not highest
											...
											;
											// this for loop mamkes everything
											// higher false and not highest
...
function generateSweeperPokemon() {
	...
	for (var i = 0; i < USAGE_LIST[tier].length; i++) {
		...
		numPokemon++;
		if (tierOptions.liteMode && numPokemon >= 200)
			break;
	}
	// get all custom sets
					...
					function() {
						...
						for (var i = 0; i < environment.pokemons[0].length; i++) {
							...
							for (var j = 0; j < environment.pokemons[1].length; j++) {
								...
								;
								results = getAttackResults(attacker, target,
										environment,
										tierOptions.enablePriority, tableMode);
								if (results[0].attacks
										&& results[0].attacks.length > 0) {
								...
								;
								if (results[1].attacks
										&& results[1].attacks.length > 0) {
							...
							if (info.outSpeeds >= tierOptions.outSpeed
								...
								// info.attacker.name + ' set: ' +
								// info.attacker.setName + ' score: ' +
								// info.total[0]);
								infoArray.push(info);
						...
						for (var i = 0; i < infoArray.length; i++) {
							...
							if (pokemon.valid
									&& isValidPokemonName(pokemon.name)) {
								$(cell_id)
										.html(
						...
						for (var i = infoArray.length; i < RMT_TABLE_LENGTH; i++) {
							...
							if ($(cell_id).html() !== '<img src="../css/images/5_5_blank.png">') {
								$(cell_id)
										.html(
												'<img src="../css/images/5_5_blank.png">');
							}
							;
						}
						;
						...
						for (var i = 0; i < environment.pokemons[1].length; i++) {
							...
							if (pokemon.valid
									&& isValidPokemonName(pokemon.name)) {
								if ($(cell_id).html() !== '<img src="../css/images/sprites/'
										+ pokemon.name + '.png">') {
									$(cell_id).html(
											'<img src="../css/images/sprites/'
													+ pokemon.name + '.png">');
								}
								;
							} else {
								if ($(cell_id).html() !== '<img src="../css/images/96_96_blank.png">') {
									$(cell_id)
											.html(
													'<img src="../css/images/96_96_blank.png">');
								}
								;
							}
							;
						...
						if (noResults) {
							...
							$('.outputLink').attr('href',
									exportToUrl(environment, tierOptions));
						}
						;
					});
 ...
 *            column of the cell
 * @param isValid
 *            true if the cell should be valid, or false otherwise.
 * @param results
 *            the results to be displayed in the cell.
 */
function updateCells(row, col, isValid, results) {
	...
	if (environment.dualMode) {
		...
		if (mainSubCell === 1) {
			updateCell(secondaryCellId, isValid, results, secondarySubCell,
					false);
		} else {
			updateCell(secondaryCellId, isValid, results, secondarySubCell,
					true);
...
function saveData() {
	...
	// localStorage.setItem('sweeperCalcEnvironment',
	// JSON.stringify(environment));
};

/**
 * Gives Pokemon some boosts based on the moves it has.
 */
 ...
 * 
 * @param attack
 *            the attack that the cell is representing
 * @param attack
 *            the opposing attack
 * @param subCell
 *            the top or bottom half of a cell
 * @param attackerStrikeFirst
 *            true if the current attacker strikes first or false otherwise.

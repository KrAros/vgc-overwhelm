 ...
 *            Name of the Pokemon
 * @param stat
 *            Base stat to be determined
 * @param attacker
 *            True if pokemon is the attacker, or false otherwise.
...
function chainMultipleModifiers(modifiers) {
   ...
   for (var i = 0; i < modifiers.length; i++) {
	  if(modifiers[i] !== 0x1000) {
		 newModifier = ((newModifier * modifiers[i]) + 0x800) >> 12;
	  }
   }
   return newModifier;
};
...
/**
 * Determines if the move is a move that targets the target's defence despite
 * being special
 */
...
function getStat(pokemon, stat, attacker) {
   ...
   if (stat === STAT_HP) {
      ...
      // getNatureMod returns either 9, 10 or 11. This prevents division too
		// early.
 ...
 * 
 * @param attacker
 *            Pokemon attacker object
 * @param target
 *            Pokemon target object
 * @param move
 *            Move object that the attacker is using.
 * @param environment
 *            Environment object
...
function getBasePower(description, attacker, target, move, environment) {
   ...
   if (environment.zMoveCurr && move.name == 'Clanging Scales' && typeof Z_CRYSTAL[attacker.item] != 'kommonium z' && attacker.item != ('life orb' || 'expert belt' || 'muscle band' 
			|| 'choice scarf' || 'choice band' || 'choice specs')) {
	    power = 185;
	}   
   
   if (environment.zMoveCurr && move.name == 'Stone Edge' && typeof Z_CRYSTAL[attacker.item] != 'lycanium z' && attacker.item != ('life orb' || 'expert belt' || 'muscle band' 
			|| 'choice scarf' || 'choice band' || 'choice specs')) {
	    power = 190;
	}
   
   if (environment.zMoveCurr && move.name == 'Play Rough' && typeof Z_CRYSTAL[attacker.item] != 'mimikium z' && attacker.item != ('life orb' || 'expert belt' || 'muscle band' 
			|| 'choice scarf' || 'choice band' || 'choice specs')) {
	    power = 190;
	}
   
    if (environment.zMoveCurr && move.name != 'Heavy Slam' && typeof Z_CRYSTAL[attacker.item] != 'undefined' 
	&& move.type == TYPE_NAME_TO_CRYSTAL[attacker.item] && attacker.item != ('life orb' || 'expert belt' || 'muscle band' 
	|| 'choice scarf' || 'choice band' || 'choice specs')) {
       
    // move.name = move.name.replace(move.name, move.zname);
	   // move.name = move.zname;
	   ...
	   else if (power >= 70 && power < 76) {
		   power = 140;
	   }
	   else if (power >= 80 && power < 86) {
		   power = 160;
	   }
	   else if (power >= 90 && power < 96) {
		   power = 175;
	   }
	   else if (power === 100) {
		   power = 180;
...
function applyBasePowerModifiers(power, description, attacker, target, move, environment) {
   ...
   if (target.ability === 'fluffy' && move.type === TYPE_NAME_TO_ID.Fire) {
      description.defenderAbility = ABILITIES[target.ability].name;
      modifiers.push(0x2000);
   };   
   if (target.ability === 'fluffy' && isContactMove(move)) {
      description.defenderAbility = ABILITIES[target.ability].name;
      modifiers.push(0x800);
   };   
   if (environment.zMoveCurr === false) {
	   if(attacker.ability === "galvanize" && move.type === TYPE_NAME_TO_ID.Normal) {
		   move.type = TYPE_NAME_TO_ID.Electric;
		   modifiers.push(0x1333);
	   };
   };
   if (move.type === TYPE_NAME_TO_ID.Steel && attacker.ability === "steelworker"){
        bpMods.push(0x1800);
   }
   ...
   if (attacker.item === 'wise glasses' && move.category === MOVE_SPECIAL && environment.zMoveCurr === false) {
      description.attackerItem = ITEMS[attacker.item].name;
      modifiers.push(0x1199);
   };
   ...
   if (move.name === 'Knock Off' &&
         /*
			 * ! (target.name === 'arceus' && typeof
			 * MULTITYPE_PLATES[target.item] !== 'undefined') && ! (target.name
			 * === 'giratina' && target.item === 'griseous orb') &&
			 */
         target.item !== null &&
         typeof MEGA_STONES[target.item] === 'undefined' && typeof Z_CRYSTAL[target.item] === 'undefined') {
...
function applyAttackModifiers(attack, description, attacker, target, move, environment) {
   ...
   if (target.ability === 'thick fat' && 
         (move.type === TYPE_NAME_TO_ID.Fire || move.type === TYPE_NAME_TO_ID.Ice) || target.ability === 'water bubble' && 
   ...
   if ((attacker.name === 'cubone' || attacker.name === 'marowak' || attacker.name === 'marowak-alola') &&
...
function getSpeed(description, pokemon, environment, team) {
   ...
   if (pokemon.status === STATUS_PARALYSE) {
      speed = applyModifier(speed, 0x800);
   };
   // apply tailwind
...
function applySpeedModifiers(description, speed, pokemon, environment, team) {
   ...
   if (ability === 'slush rush' && environment.weather === ENVIRONMENT_HAIL) {
      description[team + 'Ability'] = ABILITIES[ability].name;
      description.weather = ENVIRONMENT_ID_TO_NAME[environment.weather];
      modifiers.push(0x2000);
   };
   if (ability === 'surge surfer' && environment.terrain === ELECTRIC_TERRAIN) {
      description[team + 'Ability'] = ABILITIES[ability].name;
      description.weather = ENVIRONMENT_ID_TO_NAME[environment.weather];
      modifiers.push(0x2000);
   };
   ...
   /*
	 * if (ability === 'unburden' && pokemon.item &&
	 * pokemon.item.match(/berry|gem/i)) { description[team + 'Ability'] =
	 * ABILITIES[ability].name; modifiers.push(0x2000); };
	 */
...
function applyBaseModifiers(baseDamage, description, attacker, target, move, environment, random) {
   ...
   baseDamage = applyTerrainModifier(baseDamage, description, attacker, target, move, environment);
   // baseDamage = applyCritModifier(baseDamage, description, attacker, target,
	// move, environment);
   // todo?? Critical hits
...
function applyMultiTargetModifier(baseDamage, description, attacker, target, move, environment) {
   if (environment.doubles && MULTI_TARGET_MOVES[move.name] && environment.zMoveCurr === false) {
      baseDamage = applyModifier(baseDamage, 0xC00);
   };
   if (environment.doubles && MULTI_TARGET_MOVES[move.name] && environment.zMoveCurr === true) {
	      baseDamage;
	   };  
   return baseDamage;
};
...
/**
 * Apply terrain modifier to the damage
 * 
 * @param baseDamage
 *            Current base damage
 * @param attacker
 ...
 * @return the modified base damage.
 */
function applyTerrainModifier(baseDamage, description, attacker, target, move, environment) {

   if (environment.terrain === GRASSY_TERRAIN) {
	  ...
	  if ((attacker.ability === 'levitate' || _type === TYPE_NAME_TO_ID.Flying || attacker.item === 'air balloon') && move.type === TYPE_NAME_TO_ID.Grass ) {
            return baseDamage;
	  }
	  }
      if (move.type === TYPE_NAME_TO_ID.Grass) {
         description.terrain = TERRAIN_ID_TO_NAME[environment.terrain];
         baseDamage = applyModifier(baseDamage, 0x1800);
         } 
		  else if (move.name === 'Earthquake' || move.name === 'Magnitude' || move.name === 'Bulldoze') {
         description.terrain = TERRAIN_ID_TO_NAME[environment.terrain];
         baseDamage = applyModifier(baseDamage, 0x800);
      };
   }
    else if (environment.terrain === ELECTRIC_TERRAIN) {
	  for (var i = 0; i < POKEMON_DATA[attacker.name].type.length; i++) {
      var _type = POKEMON_DATA[attacker.name].type[i];
      // take into account freeze dry
	  if ((attacker.ability === 'levitate' || _type === TYPE_NAME_TO_ID.Flying || attacker.item === 'air balloon') && move.type === TYPE_NAME_TO_ID.Electric ) {
            return baseDamage;
	  }
	  }
      if (move.type === TYPE_NAME_TO_ID.Electric) {
         description.terrain = TERRAIN_ID_TO_NAME[environment.terrain];
         baseDamage = applyModifier(baseDamage, 0x1800);
      };
   }
    else if (environment.terrain === MISTY_TERRAIN) {
	  for (var i = 0; i < POKEMON_DATA[target.name].type.length; i++) {
      var _type = POKEMON_DATA[target.name].type[i];
      // take into account freeze dry
	  if ((target.ability === 'levitate' || _type === TYPE_NAME_TO_ID.Flying || target.item === 'air balloon') && move.type === TYPE_NAME_TO_ID.Dragon ) {
            return baseDamage;
	  }
	  }
      if (move.type === TYPE_NAME_TO_ID.Dragon) {
         description.terrain = TERRAIN_ID_TO_NAME[environment.terrain];
         baseDamage = applyModifier(baseDamage, 0x800);
      };
   }
...
function applyFinalModifiers(baseDamage, description, attacker, target, move, environment) {
   ...
   if (environment.helpingHandCurr && (move.category === MOVE_PHYSICAL || move.category === MOVE_SPECIAL) &&
         attacker.ability !== 'infiltrator') {
      description.isHelpingHand = true;
      if (environment.doubles) {
         modifiers.push(0x1000*1.5);
      } else {
         modifiers.push(0x1000*1.5);
      };
   };
   if (environment.reflectCurr && move.category === MOVE_PHYSICAL &&
         attacker.ability !== 'infiltrator' && environment.critCurr != true) {
      description.isReflect = true;
      if (environment.doubles) {
         modifiers.push(0x1000*0.6666);
      } else {
         modifiers.push(0x800);
      };
   };
   ...
   if (environment.auroraVeilCurr && (move.category === MOVE_PHYSICAL || move.category === MOVE_SPECIAL) &&
         attacker.ability !== 'infiltrator' && environment.critCurr != true)  {
   ...
   if (environment.critCurr) {
      description.isCrit = true;
      baseDamage = applyModifier(baseDamage, 0x1800);
   };
   if (target.ability === 'multiscale' || target.ability === 'shadow shield' 
   ...
   if ((target.ability === 'solid rock' || target.ability === 'filter' || target.ability === "Prism Armor")
         && getMoveEffectiveness(description, attacker, target, move, environment) >
            EFFECTIVENESS_NORMAL) {
      description.defenderAbility = ABILITIES[target.ability].name;
      modifiers.push(0xC00);
...
function applySpecialCases(baseDamage, description, attacker, target, move, environment) {
   ...
   if (environment.terrain === PSYCHIC_TERRAIN) {
	  for (var i = 0; i < POKEMON_DATA[attacker.name].type.length; i++) {
      var _type = POKEMON_DATA[attacker.name].type[i];
      // take into account freeze dry
	  if ((attacker.ability === 'levitate' || _type === TYPE_NAME_TO_ID.Flying || attacker.item === 'air balloon') && move.type === TYPE_NAME_TO_ID.Psychic ) {
            return baseDamage;
	  }
	  }
      if (move.type === TYPE_NAME_TO_ID.Psychic) {
         description.terrain = TERRAIN_ID_TO_NAME[environment.terrain];
		 specialCase = true;
         baseDamage = applyModifier(baseDamage, 0x1800);
      };
	  for (var i = 0; i < POKEMON_DATA[target.name].type.length; i++) {
      var _type = POKEMON_DATA[target.name].type[i];
	  if ((target.ability === 'levitate' || _type === TYPE_NAME_TO_ID.Flying || target.item === 'air balloon') && move.priority > 0 ){
         description.terrain = TERRAIN_ID_TO_NAME[environment.terrain];
         return baseDamage;
		 }
      };
   ...
   if (environment.protectCurr === true && environment.zMoveCurr === false) {
	   specialCase = true;
		 baseDamage = 0;
		 description.protect = true;
   }  
   if (environment.protectCurr === true && environment.zMoveCurr === true ) {
	   specialCase = true;
		 baseDamage = baseDamage*0.250;
		 description.protect = true;
   }
...
function getDamageAmount(description, attacker, target, move, environment, random) {
   ...
   // silvally
   if (attacker.ability === 'rks system' &&
	         move.name === 'Multi-Attack' &&
	         typeof MULTITYPE_MEMORY[attacker.item] !== 'undefined') {
	      description.attackerAbility = ABILITIES[attacker.ability].name;
	      description.attackerItem = ITEMS[attacker.item].name;
	      move.type = MULTITYPE_MEMORY[attacker.item];
	      description.moveType = TYPE_ID_TO_NAME[move.type];
	   };
   // weather ball
   ...
   // base damage
   var baseDamage = Math.floor(Math.floor((Math.floor((2 * level) / 5 + 2) * basePower * attack) / defence) / 50 + 2);
   ...
   if (attacker.ability === 'parental bond') {
      description.attackerAbility = ABILITIES[attacker.ability].name;
      baseDamage = (baseDamage * 0.75) + (preSturdyDamage >> 1);
   };
 ...
 *            RNG seed
 * @param multiHit -
 *            true if you want to calculate multi hit damage or false if not
...
/**
 * Returns true if the attacker is faster, or false otherwise. Takes into
 * account trick room.
 * 
 * @param attacker
 *            Pokemon attacker object
...
function getAttackResults(attacker, target, environment, enablePriority, tableMode) {
   ...
   if (tableMode !== TABLEMODE_WMT) {
      ...
      environment.auroraVeilCurr = environment.auroraVeilB;	
      environment.critCurr = environment.critA;	  
      environment.helpingHandCurr = environment.helpingHandA;	
      environment.zMoveCurr = environment.zMoveA;	  	
      environment.protectCurr = environment.protectB;	  
   ...
   if (tableMode !== TABLEMODE_BRMT) {
      ...
      environment.auroraVeilCurr = environment.auroraVeilA;
      environment.critCurr = environment.critB;
      environment.helpingHandCurr = environment.helpingHandB;	 
      environment.zMoveCurr = environment.zMoveB;	  	 
      environment.protectCurr = environment.protectA;	  
...
function getAttackResult(templateDescription, attacker, target, environment, moveName) {
   ...
   if (environment.zMoveCurr && move.name != 'Heavy Slam' && typeof Z_CRYSTAL[attacker.item] != 'undefined' 
		&& move.type == TYPE_NAME_TO_CRYSTAL[attacker.item] && attacker.item != ('life orb' || 'expert belt' || 'muscle band' 
		|| 'choice scarf' || 'choice band' || 'choice specs')) {
	   description.moveName = move.zname;
  }
 ...
 * 
 * @param pokemon1
 *            first Pokemon object
 * @param pokemon2
 *            second Pokemon object

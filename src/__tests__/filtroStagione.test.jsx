/**
 * src/__tests__/filtroStagione.test.jsx
 *
 * Che la stagione scelta filtri DAVVERO la tendina dei set.
 *
 * ─── PERCHE' NON BASTA IL TEST SUL NEGOZIO ─────────────────────────────────
 *
 * `stagioneSelezionata.test.js` prova che il valore parte giusto e che non
 * finisce nel link. Nessuno dei due tocca `PresetSelect`, che è dove il filtro
 * vive: si poteva scrivere la stagione nel negozio, leggerla nel componente e
 * poi non usarla, e tutti quei test sarebbero rimasti verdi. Qui il componente
 * si renderizza davvero e si contano le opzioni.
 *
...
// Incineroar ha un set meta di M-4: è la specie su cui si vede il filtro.
...
/**
 * Renderizza la tendina con una stagione già scelta, e restituisce le
 * etichette delle `<option>`.
 *
 ...
 */
async function opzioniCon(stagione) {
  memoria.clear()
  memoria.set('vgc-overwhelm-teams', JSON.stringify({ team1: TEAM, team2: VUOTA }))
  memoria.set('sixth_ember_stagione', stagione)
  vi.resetModules()
...
describe('la stagione filtra la tendina dei set', () => {
  it('con la stagione dei set, il set di Incineroar c\'è', async () => {
    const o = await opzioniCon('M-4')
    expect(o.some(x => /Sitrus/i.test(x)), `nessun set M-4: ${o.join(' / ')}`).toBe(true)
  })

  it('con una stagione senza set, la tendina resta senza set meta', async () => {
    // IL CASO CHE PROVA IL FILTRO. Se `PresetSelect` ignorasse la stagione,
    // qui vedremmo lo stesso elenco del test sopra.
    const o = await opzioniCon('M-1')
    expect(o.some(x => /Sitrus/i.test(x)), `set di M-4 mostrati sotto M-1: ${o.join(' / ')}`).toBe(false)
  ...
  it('con «tutte» i set tornano, e portano scritta la stagione', async () => {
    const o = await opzioniCon('tutte')
    const conStagione = o.filter(x => /·\s*M-\d/.test(x))
    expect(conStagione.length, `nessuna opzione etichettata: ${o.join(' / ')}`).toBeGreaterThan(0)
    expect(conStagione.some(x => /M-4/.test(x))).toBe(true)
  })
  ...
  it('controllo negativo: la semina arriva davvero al componente', async () => {
    ...
    const conSet = await opzioniCon('M-4')
    const senzaSet = await opzioniCon('M-1')

...
import { nomiAccessibili, ancoreInterne } from './helpers/nomiAccessibili.js'
import App from '../App'
import '../i18n'

const html = renderToStaticMarkup(<App />)
const { ancore, rotte } = ancoreInterne(html)
const esito = nomiAccessibili(html)
...
describe('ancore interne', () => {
  ...
  it('nessuna punta a un id inesistente', () => {
 ...
 * `sm:hidden` per il telefono — e la copia mobile era nata senza nomi. Il
 * markup diverso è una scelta legittima (un <select> nativo su telefono apre
 * la rotella di sistema); ciò che non lo è, è che una copia dica meno.
 *
 * L'asserzione è sulla PROPRIETÀ e non sui nove nodi: enumerarli passerebbe
 * per sempre senza dire niente del prossimo ramo `sm:hidden` senza etichette.
 *
 * ─── IL CONFINE DI QUESTO FILE, DICHIARATO ─────────────────────────────────
 *
 * Qui `App` è renderizzata con i team VUOTI, cioè la pagina d'ingresso. In
 * quello stato ci sono due <select> e ZERO cursori: l'editor non esiste.
 *
 * In P avevo scritto nel piano che questo test «asserisce la proprietà», e la
 * frase era troppo forte — la asserisce su questo stato soltanto. Con una
 * squadra caricata i controlli senza nome sono cinquanta, e li sorveglia
 * `accessibilitaEditor.test.jsx`. Trovato in P-2, contando i nodi.
 *
 * La logica di analisi è condivisa in `helpers/nomiAccessibili.js`: due copie
 * della definizione di «senza nome» sarebbero due copie della stessa
 * assunzione, ed è l'errore di `gap.test.js` archiviato in F-3.
 */
describe('controlli senza nome — pagina d’ingresso', () => {
  it('ci sono bottoni e tendine da controllare, altrimenti non si verifica niente', () => {
    expect(esito.totali.bottoni).toBeGreaterThan(10)
    expect(esito.totali.tendine).toBeGreaterThan(0)
  })
